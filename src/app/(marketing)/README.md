...
Public pages that need no account: the landing page (`/`: one line and the "Open Case 1" button) and "What we store" (`/privacy`). `layout.tsx` adds the footer, which links to both. The parentheses make this a route group, so it adds no URL segment.
