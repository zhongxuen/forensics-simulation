...
- `tokens.css` — semantic design tokens (`--surface-*`, `--text-*`, `--accent*`, `--status-*`, `--reward*`, `--border-*`, `--focus-ring`, `--term-*`) on a dark-first `:root` ("Lamplight"), and one `[data-theme="light"]` block ("Daylight", the `appTheme` setting) reassigning the same names. The `--term-*` tokens sit in a `:root`-only block that Daylight never touches: the terminal is dark in both themes, and its frame sets `data-theme="dark"` so its header and menus keep the dark tokens.
...
- `motion.css` — duration and easing tokens, every animation (`animate-pop`, `animate-tick-fill`, `animate-burst`, and UIUX.md §5's `animate-badge-pop`, `animate-line-flash`, `animate-stamp`, `animate-sweep`, `animate-shimmer`), the `fx-duration-*` and `fx-slide` transition utilities, `fx-sweep` and `fx-skeleton` (the still layers the sweep and shimmer move), and the decorative `fx-scanlines` / `fx-spark`. All motion is multiplied by `--motion-scale`, which is 0 under `prefers-reduced-motion: reduce` or inside `data-motion="reduce"`, so every effect lands on its finished, still state. `tests/unit/motion.test.ts` checks that every animation scales and that celebrations finish within 1.5s.
- `typography.css` — the seven type roles from `docs/UIUX.md` §4.2 as `@utility` classes: `type-display`, `type-page-title`, `type-section-title`, `type-eyebrow`, `type-body`, `type-small`, `type-data`. Name the job a piece of text does instead of picking a size; `/styleguide` shows each one.
