...
- `fs/` the virtual filesystem · `net/` the network model and the discovered-only topology selector · `shell/` running parsed command lines · `tools/` simulated tools and the Linux command set, with the examiner's toolset in `tools/forensics/`
- `evidence/` the forensics evidence model: disk, memory and log types, artefact refs, zoned time, schemas, the one byte layout every hash is taken of, pure MD5/SHA-1/SHA-256, a fluent test builder, and `session.ts`, the evidence a workstation has attached and each device's write-blocker (see its README)
