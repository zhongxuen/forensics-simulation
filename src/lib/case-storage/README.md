...
**What a save holds:** the terminal's command log (the lines typed, and Reset machine presses), pins, notes, the report draft, objectives ticked, hint tiers shown and story lines played. **Never the engine's state:** opening the case replays the log through the engine (`src/features/cases/run/`), so a save can't describe a machine the engine wouldn't make.
