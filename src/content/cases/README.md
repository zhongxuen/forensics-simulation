...
| `noise`                             | A `profile` and a `density` of ordinary activity around the story. Turning it up makes a case harder without touching the story. `weekends: false` keeps it to weekdays, for a case whose story turns on who was in at the weekend                                                                                              |
...
| `documents`                         | Paperwork besides the letter and the handover form, as `file` and `content`: a message from the client, a printout of a door log. It lands in the case folder for the player to read. Like the letter it is a realistic document, not game copy, and no answer can cite it                                                      |
...
| `report`                            | The questions that close the case. Each has an `answer`, an `explain`, and `acceptedEvidence`: the evidence an answer has to point at. A choice question can carry `feedback`: a cast member's line for each of the other choices, shown on the debrief when that one is picked. That is how a **choice beat** works            |
...
`overwrite-clusters` takes `keep: <n>` to leave the deleted file's first `n` clusters alone: the file is **partly** overwritten, so `recover` refuses it and `carve` finds its start with no end (Case 2's statement).
...
acceptedEvidence:
  ...
  - "disk:qf-lt-07:carve/*QF-INV-0410*" # what the carver finds: by type, "partial pdf", or text
```
...
| `jory`                         | 2026-09-24 | The bookkeeper's account in `case-02.yaml`, like `mara` a first name standing for nobody. Searched with "bookkeeper": nothing but an unrelated author, Jory John. Copy calls the person "the bookkeeper"; the account is the only identifier |
| Brackensett Joinery            | 2026-09-24 | Case 2's customer, who ends up with the price list, and whose computers are out of scope. "Brackensett" returned no organisation, only the word bracken and unrelated places                                                                 |
| Cobbenhale Builders            | 2026-09-24 | A customer on Case 2's price list and first invoice. "Cobbenhale" returned nothing; the nearest were Cobbinshaw and Sedgehill, both places                                                                                                   |
| Sedgewhistle Packaging         | 2026-09-24 | A customer on Case 2's price list and second invoice. "Sedgewhistle" returned nothing; the nearest was the sedge warbler                                                                                                                     |
| YardBackup, `yard-backup.exe`  | 2026-09-24 | The yard's nightly backup program in Case 2. No product of that name was found, only Yadis! Backup and the Ruby documentation tool YARD                                                                                                      |
| `parcel-drop.example`          | —          | The name the office laptop looked up before the upload in Case 2. Under `.example`, so it can never be anybody's                                                                                                                             |
| `dispatch-admin`, `svc-update` | —          | Case 3's accounts, both named in `docs/plan/12-case-3-something-is-still-running.md`: the dispatch server's real administrator account, and the one added on Sunday night. Role names standing for nobody                                    |
| `WIN-7Q2LD0`                   | —          | The machine name Case 3's remote sign-in came from, in the shape Windows gives a machine nobody named. It is part of a record, not a place in the world, and stands for nobody                                                               |

Five names were searched for Case 2 and dropped: **Tallowbeck Joinery** (Tallow Build is a real building and joinery firm in York, and a joinery is exactly what the name was for), **Harbour Road Builders** (Harbour Town Builders and Harbour Roads, a paving contractor, are real), **Millrace Paper** (a letter away from Millcraft Paper, a real paper distributor), **Fallowmere** (a World of Warcraft location and a place in a published fantasy series) and **Corriebrae** (a real address in Dumfriesshire that companies are registered at).
