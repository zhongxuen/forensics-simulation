...
Plan file 13 (the Learning Center) is part-built: prompt 13.1 wrote the Foundations track — what forensics is, order of volatility, chain of custody, hashing for evidence — with `src/content/tracks.ts`, the citations in `src/content/references.ts`, and `externalPrerequisites` linking Hacker Simulation's lessons. Prompt 13.2 wrote the Disk track — partitions and file systems, MACB times, deleted versus overwritten, carving — on **generated practice evidence**: `src/content/practice/stories.ts` holds a story per practice drive (Candlewright's TRAIN-07 stick and practice laptop `train-lt-01`, baseline `training-laptop-v1`), `pnpm evidence:build` plays them into `src/content/practice/<id>.evidence.json`, and a practice machine in `mini-terminals.ts` names one in `evidence` (with optional `prepare` commands), which `<MiniTerminal>` attaches under `/dev/evidence` (`src/features/learning/practice/`). The hashing lesson now images TRAIN-07 and runs `hashsum --verify` against its form. Each disk tool's `help.lesson` names its lesson, which `man`, `--help` and `/learn/commands` show. Prompt 13.3 adds the Memory, and Logs and timelines tracks (and the lessons for `pin`, `logq` and `timeline`).
