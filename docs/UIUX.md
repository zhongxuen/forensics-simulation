...
Status: **every prompt from UX.1 to UX.9 has merged**, and §2.9's follow-ups are closed except O5, an owner step (recording Linux screenshot baselines). §2's items are ticked ✓ with the prompt that did each. UX.8 (2026-09-26) added `tests/e2e/visual.spec.ts` (screenshot baselines) and `tests/e2e/viewport.spec.ts`, and the UI review checklist in `docs/runbook.md` §9.
...
Found in UX.8's review of the visual baselines (`tests/e2e/visual.spec.ts-snapshots/`) and its machine checks, and closed on 2026-09-26 after UX.9. The Windows baselines were recorded again for the screens that changed. Only O5 still needs a person.

| #   | Sev | What was wrong                                                                                                                                                                                                  | Rule                     | What changed                                                                                                                                                                                                                                                                    |
| --- | --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| O1  | P2  | Two solid amber buttons on one workspace screen: the Now strip's **Write your report** and the Evidence Browser's empty-state **Open qf-lt-03**                                                                  | §3.3 rule 6, §4.3 rule 1 | ✓ The Evidence Browser's Open and "Show the files in…" buttons are secondary (`files-view.tsx`, `image-detail.tsx`), so the Now strip carries the workspace's one primary. The panes' Pin buttons stay primary: they're the action on a chosen record                          |
| O2  | P2  | The Timeline's segmented controls filled their selected option with solid amber (**Tracks**, **UTC**), so a state read like a button                                                                             | §3.3 rules 1 and 3       | ✓ The selected option sits on `--accent-subtle` with an amber inset ring (`timeline-view.tsx`, `Segmented`)                                                                                                                                                                     |
| O3  | P2  | On a 360 × 780 phone the Now strip took about 45% of the height                                                                                                                                                 | §4.3 rule 2 (and §2.5)   | ✓ Below `sm`, Noor's newest message is one line beside its "n earlier messages" button (`now-strip.tsx`), which saves about three lines                                                                                                                                         |
| O4  | P3  | On a phone the breadcrumb cut the case title short ("Cases / The cl…")                                                                                                                                          | §4.3 rule 2              | ✓ Below 480 px the section crumb and its slash are hidden, so the title shows whole (`breadcrumb.tsx`)                                                                                                                                                                           |
| O5  | P2  | The visual baselines are Windows pictures, so `visual.spec.ts` skips on CI's Linux runner                                                                                                                       | §8 "UX.8 visual review"  | **Owner step.** `.github/workflows/visual-baselines.yml` (run it by hand: Actions → Visual baselines) records the Linux pictures and uploads them as an artifact. Review them and commit them to `tests/e2e/visual.spec.ts-snapshots/` (`docs/runbook.md` §9) |
| O6  | P3  | `perf:vitals` counted 205.9 KB of script by the load event on `/cases/case-01`, over its 200 KB line, while `bundle:check` had the page within budget                                                            | §8 "Initial JS per page" | ✓ Not a deferred chunk: the page fetches exactly the 15 chunks `bundle:check` counts. The gap was each response's HTTP headers (about 17 KB in all). `perf:vitals` now counts compressed bodies only ("JS (gzip)"): 189.3 KB, against `bundle:check`'s 188.6 KB |
...
UX.1 then applied the rest: the logo mark, the "letter" and "evidence tag" card styles, the type scale, and the SIMULATED badge's single-instance rule. UX.9 added the light theme (§3.5).
