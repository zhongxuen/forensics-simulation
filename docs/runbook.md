...
Update its baseline and look at it: `pnpm build && pnpm test:e2e tests/e2e/visual.spec.ts --update-snapshots`, then open the changed pictures in `tests/e2e/visual.spec.ts-snapshots/` and check them against this list before committing. The committed baselines were recorded on Windows; the spec skips on a platform without its own (CI's Linux runner). **Owner step:** record and commit Linux baselines once, so CI compares them too: run the **Visual baselines** workflow by hand (Actions → Visual baselines → Run workflow, `.github/workflows/visual-baselines.yml`), download its `visual-baselines-linux` artifact, check each picture against this list, and commit them into `tests/e2e/visual.spec.ts-snapshots/`. (A Linux machine, or Playwright's Docker image `mcr.microsoft.com/playwright`, works too.)
